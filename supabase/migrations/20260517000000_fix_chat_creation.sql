-- 20260517000000_fix_chat_creation.sql
-- Creates an RPC to safely start a conversation, bypassing RLS and ensuring profiles exist
...
BEGIN
  ...
  VALUES (p_tenant_id, 'Tenant', 'student')
  ...
  -- Ensure listing exists check (optional, but good for integrity)
  -- Foreign key will catch it if room doesn't exist, but let's just proceed.
