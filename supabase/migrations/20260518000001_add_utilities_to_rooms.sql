-- 20260518000001_add_utilities_to_rooms.sql
-- Add utility information fields to the rooms table
-- These fields are informational only - they tell tenants what to expect
-- All utility payments are handled directly between tenant and landlord

ALTER TABLE public.rooms
  ADD COLUMN IF NOT EXISTS elec_type   text DEFAULT 'shared'  CHECK (elec_type  IN ('independent', 'shared', 'included')),
  ADD COLUMN IF NOT EXISTS water_type  text DEFAULT 'shared'  CHECK (water_type IN ('independent', 'shared', 'included')),
  ADD COLUMN IF NOT EXISTS waste_cost  int  DEFAULT 0;        -- Monthly waste collection fee in TZS (0 = not charged)
