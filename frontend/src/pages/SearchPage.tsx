...
// ── Module-level coordinate helpers (stable across renders) ─────────────────
...
/**
 * Generate a deterministic sub-ward offset from a listing's UUID so that
 * listings sharing the same ward centre don't all stack on one point.
 * Offset is ±0.0025° ≈ ±275m — stays inside the ward, clearly readable.
 */
function stableJitter(id: string): { dlat: number; dlng: number } {
  let h = 2166136261;
  for (let i = 0; i < id.length; i++) {
    h ^= id.charCodeAt(i);
    h = Math.imul(h, 16777619);
  }
  const u = (h >>> 0) / 0xffffffff;          // 0..1 from first hash
  const v = ((h >> 8) >>> 0) / 0xffffffff;   // 0..1 from shifted hash
  return {
    dlat: (u - 0.5) * 0.005,   // ±0.0025°
    dlng: (v - 0.5) * 0.005,
  };
}

/** Returns exact GPS or ward-centre + stable jitter. Returns null if no match. */
function getApproxCoords(listing: any): { lat: number; lng: number } | null {
  // Prefer exact GPS stored in the DB
  if (listing.lat && listing.lng) {
    return { lat: Number(listing.lat), lng: Number(listing.lng) };
  }
  const ward = (listing.ward || listing.district || '')
    .toLowerCase()
    .replace(/[\s-]+/g, '_');
