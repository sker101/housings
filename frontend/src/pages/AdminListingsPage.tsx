...
export default function AdminListingsPage() {
  ...
  const openModal = (action, targetId = null, isBulk = false) => {
    if (action === 'approve' || action === 'unflag') {
      if (isBulk) {
        processBulkAction(action, null);
      } else {
        processAction(targetId, action, null);
      }
      return;
    }
  ...
  return (
    <div className="container section">
        ...
        {pendingOrFlagged.map((listing) => (
          <article key={listing.id} className="moderation-card">
            ...
            <div className="moderation-card__actions">
              <button className="btn" onClick={() => openModal('approve', listing.id, false)}>
                Approve
              </button>
              <button className="btn btn--secondary" onClick={() => openModal('reject', listing.id, false)}>
                Reject
              </button>
              {listing.status !== 'flagged' ? (
                <button className="btn btn--danger" onClick={() => openModal('flag', listing.id, false)}>
                  Flag
                </button>
              ) : (
                <button className="btn btn--ghost" onClick={() => openModal('unflag', listing.id, false)}>
                  Unflag
                </button>
              )}
            </div>
      ...
      <ModerationModal
        ...
        targetText={
          modalState.isBulk
            ? `${selectedIds.size} selected listings`
